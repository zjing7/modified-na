...
def convert_pdb_xyz(pdbfile, xyzfile, tpfile, use_element=False):
...
def convert_xyz_pdb(pdbfile, xyzfile, tpfile):
    d = read_template(tpfile)
    d2 = {}
...
def xyz_2_lib(fprm, fxyz, flib):
    ...
    for i in range(1, t.top_natom+1):
        ...
        if atype in dprm:
            ...
            i2 = idxs[-2]
            print(i1, i2)
            outl += dprm[atype][:i1] + ' "ligand" ' + dprm[atype][i2:]
        else:
            return
...
def main():
    parser = argparse.ArgumentParser(prog='pdbxyz.py', usage='%(prog)s mode [options]', description=__doc__)
    parser.add_argument('mode', nargs='?', metavar='mode', choices=['gen', 'conv', 'pdb', 'xyz2lib'], help='[gen|conv|pdb] gen: Generate library file from PDB and XYZ; conv: Convert PDB to XYZ using library file; pdb: Convert XYZ to PDB')
    ...
    if v.mode == 'gen':
        ...
        for i in range(min(len(v.p), len(v.x))):
            reslib.update(train_tplt(v.p[i], v.x[i], biotype))
        ...
        for res in (sorted_res):
            ...
            lastres = res
        if v.app:
            with open(v.t, 'a') as fout:
                print(outp, file=fout)
        else:
            print(outp)
    elif v.mode == 'conv':
        for i in range(min(len(v.p), len(v.x))):
            convert_pdb_xyz(v.p[i], v.x[i], v.t)
    elif v.mode == 'pdb':
        for i in range(min(len(v.p), len(v.x))):
            convert_xyz_pdb(v.p[i], v.x[i], v.t)
    elif v.mode == 'xyz2lib':
        xyz_2_lib(v.k, v.x[0], v.t)
